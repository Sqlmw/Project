...
# ==================== 3. 蛋白序列相似度划分 ====================
print("3/5 蛋白序列提取与k-mer聚类（替代cd-hit）...")
...
# ==================== 4. 蛋白家族外推 ====================
print("4/5 蛋白家族外推（激酶识别）...")

def get_title(pdb):
    pdb_file = os.path.join(refined_dir, pdb, f"{pdb}_protein.pdb")
    try:
...
df['title'] = df['pdb'].apply(get_title)
df['is_kinase'] = df['title'].str.contains('kinase', case=False, na=False)
n_kinase = df['is_kinase'].sum()
print(f"   识别到激酶复合物: {n_kinase}")

kinase_test = df[df['is_kinase']].index.values
if len(kinase_test) > 10:
    splits['Family (kinase)'] = kinase_test
    print(f"   测试集（激酶外推）: {len(kinase_test)}")
else:
    print("   警告：激酶太少，跳过此划分")
